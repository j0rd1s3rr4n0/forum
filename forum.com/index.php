...
<html>
   ...
   <body>
      ...
      <div class="title-wrapper">
         <h1 class="bluegraded title-page">POSTS RECIENTES</h1>
      </div>
      <div class="container">
         <div class="wrapper">
            <?php
               ...
               require_once(__ROOT__.'db.php');
                ...
                $sql = "SELECT id_post,username,TEXT,TITLE,private,(points-(reports*5)) as 'likes' FROM posts INNER JOIN usr ON usr.id_usr = posts.user WHERE private = 0 ORDER BY id_post ASC;";
                            ...
                            xmlHttp.onreadystatechange = function()
                            {
                                if(xmlHttp.readyState == 4 && xmlHttp.status == 200)
                                {
                                  if(type == 0){
                                    ...
                                    let idd = el.parentNode.parentNode.parentNode.parentNode.childNodes[1].childNodes[1].id.replace('post','');
                                    let aab = "#aagrats_"+idd;
                                    if (document.querySelector(aab) !== null) {
                                        console.log(document.querySelector(aab).parentNode.parentNode.parentNode.parentNode.childNodes[3].childNodes[1].childNodes[1].childNodes[0].textContent);
                                        document.querySelector(aab).parentNode.parentNode.parentNode.parentNode.childNodes[3].childNodes[1].childNodes[1].childNodes[0].textContent = newtext;
                                      }
                                    //el.childNodes[2].replaceData(0,1000,newtext);
                                    //console.log(el);
                                    //console.log(xmlHttp.responseText);
                                    let abb = "#acount_"+idd;
                                    if (document.querySelector(abb) !== null) {
                                      document.querySelector(abb).click()
                                    }
                                  }
                                  if(type == 1 || type== 3 ){
                                    deletepost(el);
                   ...
                   <div class="carteta">
                     ...
                     <div class="card text-white bg-dark card-margin" id="'.$id_div_title.'">
                       <div class="card-header" id="post'.$row["id_post"].'">
                           ...
                           if((($row["username"] == $_SESSION['usr'] && $row['username'] == $username) || ($datos_mod['ismod']==1))){ 
                             ...
                             if(isset($_SESSION['usr_id'])){
                               if($datos_mod['ahide']==1 || $row['username'] == $_SESSION['usr']){
                                   echo '
                                     <a id="ahide_'.$row["id_post"].'" alt="#'.$row["id_post"].'" class="editbtn ebtn_hide" title="HIDE POST" onclick="myFunction(1,this);">
                               ...
                               if($datos_mod['aedit']==1 || $row['username'] == $_SESSION['usr']){
                                 echo '
                                   <a id="aedit_'.$row["id_post"].'" alt="#'.$row["id_post"].'" class="editbtn" title="EDIT POST" onclick="myFunction(2,this);">
                                       <i class="fa fa-pen" aria-hidden="true"></i>
                                     </a>';
                               }   
                               if($datos_mod['adelete']==1 || $row['username'] == $_SESSION['usr']){
                                 echo '
                                     <a id="adelete_'.$row["id_post"].'" alt="#'.$row["id_post"].'" class="editbtn ebtn_trash" title="DELETE POST" onclick="myFunction(3,this);">
                         ...
                         .orange:hover{color:orange !important;}
                         ...
                         <div style="/*background-color:green;*/width:100%;">
                           <div style="float:left;color:white;margin-left:2%;padding:15px;">
                            <a class="vote orange" name="countrep_'.$row["id_post"].'" style="text-decoration:none;" onclick="myFunction(0,this); /*location.reload();*/" id="acount_'.$row["id_post"].'"> ';
                           ...
                           <div style="float:right;">
                             <div>
                             <a id="aagrats_'.$row["id_post"].'"onclick="myFunction(4,this);" name="likebtn_'.$row["id_post"].'"><i class="fas fa-hands-clapping vote green like" title="Gratitude"></i></a>
                             <a id="aabuse_'.$row["id_post"].'" onclick="myFunction(5,this);"><i class="fas fa-flag vote red report" title="Report Abuse"></i></a>';
                               if($datos_mod['aban']==1){                              
                                 echo '<a id="'.$row["id_post"].'" onclick="myFunction(6,this);"><i class="fa-solid fa-gavel vote purple ban" title="Ban Post"></i></a>';
                               }
                             echo '</div>
                           </div>
                         </div>
